...
class DropBoxController{
    ...
    constructor(){
        //currentFolder() = pastaAtual()
        this.currentFolder = ['janiel'];
        //OnSelectionChange() = naMudançaDeSeleção( mudança de seleçao dos meus arquivos)
        this.onselectionchange = new Event('selectionchange');
        //navE1() = navegação
        this.navE1 = document.querySelector('#browse-location');
        //btnSendFileE1 = botaoMandarArquivos
        this.btnSendFileE1 = document.querySelector("#btn-send-file");
        //inputFilesE1 = entradaArquivos
        this.inputFilesE1 = document.querySelector("#files");
        //sanckModalE1 = modal da barra de progresso
        this.snackModalE1 = document.querySelector("#react-snackbar-root");
        //progressBarE1 = progressoBarraE1
        this.progressBarE1 = this.snackModalE1.querySelector('.mc-progress-bar-fg');
        //nome do arquivo
        this.nameFileE1 = this.snackModalE1.querySelector('.filename');
        //timeLeft = temporestante
        this.timeLeftE1 = this.snackModalE1.querySelector('.timeleft');
        //listFiles = listaArquivos
        this.listFilesE1 = document.querySelector("#list-of-files-and-directories");
        //btnNewFolder() = botaoNovaPasta
        this.btnNewFolder = document.querySelector("#btn-new-folder");
        //btnRename() = botaoRenomear
        this.btnRename = document.querySelector("#btn-rename");
        //btnDelete() = botaoDeletar
        this.btnDelete = document.querySelector("#btn-delete");
        //connectFirebase = conectarFirebese
    ...
    connectFirebase(){
        ...
        //ATENÇAO CASO VOCE QUEIRA TESTA ESTE SOFTWARE, VOCÊ PRECISSA CRIAR UMA CONTA NO 
        //FOREBASE E COLOCAR OS LINKS GERADO AQUI PARA SE CONECTAR AO FIREBASE, 

    }
    //obterSelecao = para men retornar todos que estiver selecionado
    getSelection(){

        return this.listFilesE1.querySelectorAll('.selected');

    }

    ajax(url, method = 'GET', formData = new FormData(), onprogress = function(){}, onloadtart = function(){}){
        ...
        return new Promise((resolve, reject)=>{
            ...
            //startUploadTime = começoEnvioTempo 
            onloadtart();

            //e o que enviar as informações via ajax
            ajax.send(formData);
    ...
    //para remover meu arquivo
    removeTask(){

        let promises = [];

        this.getSelection().forEach(li =>{
                        //pegar todos os dados do meu arquivo li
            let file = JSON.parse(li.dataset.file);
            //pegar a chave do meu arquivo
            let key =  li.dataset.key;
            //FormData = os dados que eu quero mandar 
            let formData = new FormData();
                            //path = o nome do campo
    ...
    initEvents(){
        ...
        this.btnNewFolder.addEventListener('click', e=>{
          ...
          let name = prompt("criar uma nova pasta");
          ...
          if(name){
            ...
            this.getFirebaseRef().push().set({
                name,
                type : 'folder',
                parse:this.currentFolder.join('/')//caminho da minha pasta
            })
          }
        })
        ...
        this.btnRename.addEventListener('click', e=>{
          ...
          //para pegar os dados do meu arquivo selecionado
          let file = JSON.parse(li.dataset.file);
          let name = prompt("Renomer o Arquivo ?", file.name);
          if(name){

            file.name = name;    //child = filho que tenha a mesma chave
            this.getFirebaseRef().child(li.dataset.key).set(file);

          }

        });
        this.listFilesE1.addEventListener('selectionchange', e=>{
        ...
        this.btnSendFileE1.addEventListener('click', event=>{
            ...
            this.inputFilesE1.click();

        });
  
        this.inputFilesE1.addEventListener('change', event=>{
            this.btnSendFileE1.disabled = true;
            //para ver arquivo que foi selecionado
            // para enviar as informações do arquivo
            this.uploadTask(event.target.files).then(responses =>{

                responses.forEach(resp =>{
                    //para pegar a informação do meu arquivo = resp.files['input-file']
    ...
    uploadComplet(){
        ...
        //para zerar o campo se usuario for selecionar outro
        this.inputFilesE1.value = '';
        this.btnSendFileE1.disabled = false;

    }

    //getFirebaseRef = obterFirebaseReferencia
    getFirebaseRef(path){
    ...
    //modalShow = mostre modal (a barra de Progresso)
    modalShow( show = true){

        this.snackModalE1.style.display = (show) ? 'block' : 'none';

    }
    // uploadTask = enviotarefa(enviarArquivos)
    uploadTask(files){
        ...
        [...files].forEach(file=>{
            ...
            //Estou chamando o meu ajax de formar dinamica
            promises.push(this.ajax('/upload','POST', formData, ()=>{
                //uploadProgress = envio progresso
                this.uploadProgress(event, file);
           }, ()=>{

               this.startUploadTime = Date.now(); //a hora que começou Upload

           }));

        });

        //para gerenciar as promises dentro do meu array
        return Promise.all(promises);
    ...
    uploadProgress(event, file){
        ...
        //loaded = carregados (dados enviados);
    ...
    //formatTimeToHuman = formatando tempo para humano
    ...
    //getFileobterView = obterVisualizaçãoIconeArquivo
    getFileIconView(file){ //para criar o icone do meu arquivo
    ...
    //obterVisualizaçãoArquivo
    getFileView(file, key){ //para retornar o meu arquivo

        let li = document.createElement('li');
        //passar tembem uma "chave" pro meu elemento para indentificar
        li.dataset.key = key;
        //para pegar dados do meu arquivo
        li.dataset.file = JSON.stringify(file);

        li.innerHTML = `
         <!--getFileIconView = obterVisualizaçãoIconeArquivo-->
    ...
    //redFiles = lerArquivos(ler Arquivos do firebase)e mostrar os arquivos
    //pegar todos os dados do firebase e refletir na tela
    readFiles(){
                            //on = ouvindo  snapshot = fotografia
        ...
        this.getFirebaseRef().on('value', snapshot =>{
            ...
            snapshot.forEach(snapshotItem =>{
                ...
                if (data.type) {
                                 //appendChild = vai adicionar elemntos não html
    ...
    //abrir a minha pasta
    //ler os arquivos que estao dentro da minha pasta
    openFolder(){
                // desligar as ultimas pasta 
    ...
    renderNav(){
      ...
      this.navE1.innerHTML = nav.innerHTML;

      this.navE1.querySelectorAll('a').forEach(a=>{
    ...
    initEventsLi(li){
        ...
        li.addEventListener('dblclick', e=>{
            ...
            switch (file.type) {
                //ler os arquivos que estão dentro da pasta
                case 'folder':
        ...
        li.addEventListener('click', e=>{
            ...
            if(e.shiftKey){//para pegar os indices
               //firstLi = primeiroLi (primeiro li que eu clickei)
               let firstLi = this.listFilesE1.querySelector('.selected');
               ...
               if(firstLi){
                   ...
                   let indexEnd;
                    ...
                    //avisar que aconteceu uma mudança de seleção
                    this.listFilesE1.dispatchEvent(this.onselectionchange);
                    return true;
            ...
            if(!e.ctrlKey){
                this.listFilesE1.querySelectorAll('li.selected').forEach(el =>{
